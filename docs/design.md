...
│   │   ├── rooms/<room-id>/        # one folder per room; any filename works
│   │   └── venue/                  # general property shots
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-08 | **Polish + per-room photo folders.** A folder per room under `assets/photos/rooms/`; the pipeline now walks sub-folders and each card picks up whatever lands in its own, falling back to a placeholder. That placeholder is transparent now — an arch echoing the logo — so an empty slot reads as empty instead of a grey box. Scroll reveals via a `Reveal` component that only ever applies its hidden state from JS (prerendered HTML must never ship `opacity:0`), CSS-keyframe hero entrance and drift, card hover lift and image zoom, and a Dormire/Festeggiare/Esplorare overview filling the gap between hero and FAQ. |
